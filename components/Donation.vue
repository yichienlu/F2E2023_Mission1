...
<template>
  <div class="mb-6 lg:mb-0 p-4 pt-2 xl:p-12 xl:pt-2 grow overflow-auto lg:flex gap-8">
    <div class="basis-5/12 2xl:basis-7/12 rounded-3xl bg-donation flex lg:flex-col p-6 xl:p-16 justify-between items-stretch grow mb-6 lg:mb-0">
      <div class="lg:w-full whitespace-nowrap">
        <h2 class="text-primaryTheme font-bold mb-4 leading-normal sm:text-xl md:text-2xl 2xl:text-4xl">您的小筆捐款<br />是每隻毛孩未來的大大動力！</h2>
        <p class="text-sm">目前小額贊助總金額</p>
        <p class="text-xl font-bold leading-normal">{{ donation.total }}</p>
      </div>
      <div class="grow flex justify-center items-center bg-[url('~/assets/images/donate.png')] bg-no-repeat bg-center bg-contain lg:bg-auto"></div>
    </div>
    
    <div class="basis-7/12 2xl:basis-5/12 overflow-auto ">
      <div v-if="!donated" class="flex flex-col h-full">
          ...
          <li>
            ...
            <label for="donate-4" class="block rounded-2xl border-2 border-gray-200 hover:bg-colorTheme2 px-4 py-6">
              ...
              <div class="flex items-center ps-4 rounded-lg bg-gray-100">
                ...
                <input type="number" placeholder="輸入金額" min="1" class="block w-full px-4 py-2 bg-transparent grow text-2xl font-bold ">
              </div>
            </label>
          </li>
        </ul>
        <button class="mt-auto font-bold text-white bg-primaryTheme w-full rounded-full py-6 px-8 hover:bg-primary duration-300" @click="donated=true">
          前往捐款
        </button>
      </div>
